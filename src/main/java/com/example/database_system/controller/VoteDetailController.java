...
public class VoteDetailController {
    ...
    //无需鉴权 这里记得放行
    @GetMapping("/{voteId}")
    public ResponseMessage<AbstractMap.SimpleEntry<Vote, List<VoteOption>>> getVote(@PathVariable UUID voteId) {
        return voteService.getDetailVote(voteId);
    }

    //提交投票 这里要鉴权
    @PostMapping("/{voteId}")
    public ResponseMessage<String> voteFor(@PathVariable UUID voteId, @RequestBody UUID voteOptionId, HttpServletRequest request) {
        //这里需要先鉴权
        String token = request.getHeader("Authorization").replace("Bearer", "");
        UUID userId = jwtUtils.getIdFromToken(token);
        return voteService.voteFor(voteId, voteOptionId, userId);
    }
