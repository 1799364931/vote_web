...
本项目是一个基于``Spring Boot``的投票系统，旨在提供一个简单易用的在线投票平台。用户可以注册、登录、创建投票、参与投票，并查看投票结果。系统采用前后端分离的架构，前端采用HTML、CSS和JavaScript实现，后端使用Spring Boot框架。
...
#### 3.2.5 数据库安全
为了保证数据库的安全性，采用以下措施：
+ 对连接数据源用户进行最小权限控制，只授予必要的权限。
```sql
create user 'vote'@'localhost' identified by 'password';
grant select,update,insert on vote_system.* to 'vote'@'localhost';
```
+ 对敏感数据进行加密处理，对用户的密码进行哈希加密存储，使用``BCrypt``算法进行加密。
+ 使用``DTO``类来传输数据，避免直接暴露实体类，防止敏感数据泄露。
...
values
  (UUID_TO_BIN('ccb873da-03e9-427f-9253-2560fd65f528'), false, '选择你喜欢的书籍进行投票吧！', '2025-06-12 01:01:00', '2025-06-10 11:56:00', '书籍投票', UUID_TO_BIN('812d1268-3389-428f-8d9e-3546f546a03c'));
...
values
  ('巴黎圣母院', 1, UUID_TO_BIN('812d1268-3389-428f-8d9e-3546f546a03c'), 0,UUID_TO_BIN('e60aff5a-e445-4379-98af-35afa6d971ea'));
...
values
  (1, UUID_TO_BIN('31000000-0000-0000-0000-000000000000'),UUID_TO_BIN('812d1268-3389-428f-8d9e-3546f546a03c'), UUID_TO_BIN('767067a8-8a1e-4e23-8ca6-d46349682be3'));
...
values
  ('CREATE', UUID_TO_BIN('ccb873da-03e9-427f-9253-2560fd65f528'), '2025-06-10 11:57:40',' 812d1268-3389-428f-8d9e-3546f546a03c', UUID_TO_BIN('dbe4e8c0-b607-462e-b244-bb7ed8cfc9b0'));
commit;
```
## 6 总结
